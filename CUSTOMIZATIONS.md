...
| `frontend/src/components/Modals/DoctypeModal.vue` | `// TATVA:` +1 import, `<Dialog>` → `<ResponsiveDialog>` (same `#body` slot) | The generic create/edit modal — Add Call and every other doctype — was a centered Dialog on mobile. The tag swap (UI.md rule 22) makes it a bottom sheet on the PWA and leaves desktop byte-for-byte stock. One path: no per-doctype sheet variants |
