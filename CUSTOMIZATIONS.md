...
| `frontend/src/components/ViewControls.vue` | `// TATVA:` the export dialog's row ceiling reads `exportJob.rowLimit` instead of a local `computed` | The ceiling is a fact about an EXPORT, not about this screen: the Smart View download states the same number, and two copies of `window.sysdefaults.max_report_rows` would drift the day one is edited. It now lives in `tatva/useExportJob`, which already owns the export lifecycle both surfaces share, and matches what the server enforces (`tatva_connect.exports.row_cap`) |
| `frontend/src/components/ListBulkActions.vue` | `// TATVA:` a `hideConvert` flag on the `options` prop, the same shape as `hideEdit`/`hideDelete`/`hideAssign` | Smart Views run the SAME bulk pipeline as the native lists and mount this component unchanged, but a Smart View is a saved question over leads, not the leads list — Convert to Deal belongs to the record's own list. One more flag on the prop that already carries three keeps that a caller's choice instead of a second component. Absent ⇒ 100% stock |
| `frontend/src/components/ListViews/ListRows.vue` | `// TATVA:` an optional `scrollKey` prop; the remembered scroll offset keys on it, falling back to `doctype` | Every Lead Smart View and the native leads list are all `doctype="CRM Lead"`, so one `useStorage('scrollPosition' + doctype)` entry was shared between all of them — scrolling a Smart View to row 400 and then opening Leads jumped it there. Unset ⇒ the old key exactly, so every stock caller is byte-for-byte unchanged |
| `frontend/src/components/FadedScrollableDiv.vue` | `// TATVA:` the final reset compares `>=` rather than `==` | The branch above it fades the bottom edge whenever `scrollTop == 0`, and the reset that should cancel it demanded the box and its content be EXACTLY equal. One padding pixel of difference and a list that cannot scroll is drawn half-faded — visible on a Smart View condition row with a single condition in it |
...
| `frontend/src/components/Activities/CallArea.vue` | `// TATVA:` +1 `showMenu` prop (default true) and a `Delete` overflow over `frappe.client.delete` behind the native confirmation; the rail passes `:show-menu="false"` | Every other card carries the same overflow and `CRM Call Log` grants delete to the same three roles as `CRM Task` and `FCRM Note`, so refusing it here was an inconsistency, not a safeguard. The rail strips the overflow from every card it draws and this is the one it does not build itself, hence a prop |
...
| `frontend/src/components/Layouts/DesktopLayout.vue` | `// TATVA:` +1 import + `<TatvaBanner />` above `<AppHeader />` | Frappe renders `Website Settings.banner_html` above the navbar of every server-rendered page (`templates/base.html`), which is why the login screen carries a maintenance notice; this app serves its own document, so the shell reads the same message off the boot bag (`tatva_connect.api.banner`) and renders it in the same place. Renders nothing when there is no notice |
| `frontend/src/components/Layouts/MobileLayout.vue` | `// TATVA:` +1 import + `<TatvaBanner />` above `<MobileAppHeader />` | The mobile half of the same notice — one component, both shells |
...
| `crm/integrations/api.py` | `# TATVA:` +1 import, +1 `@request_cache` on `get_contact_by_phone_number` | `parse_call_log` calls it once per call row, at a `Contact Phone` + `CRM Lead` read each: measured 101 + 100 queries to paint a 100-row page, and 254 ms vs 46 ms against the same page with no calls on it. A lead's calls share one or two numbers, so the request memo collapses them to one lookup. Every caller is a pure lookup — none creates a record then re-reads it in the same request — and `request_cache` no-ops outside a request, so jobs are unchanged |
| `frontend/src/components/Activities/Activities.vue` | `// TATVA:` ONE actor reader (`sender()`) across every rail adapter, reading the `owner_name`/`owner_kind` the server names once per page; the WhatsApp actor is chosen by DIRECTION (inbound = the patient's `profile_name`/number, outbound = the workflow or the sender) | Five adapters named an actor five ways, and `profile_name` is the provider's own account label on an outbound row — so identical rows read "API Token 360078" and "Administrator". Server brain: `tatva_connect.activity.actor` |
| `frontend/src/components/Activities/Activities.vue` | `// TATVA:` the rail's sentence: a task names who CREATED it and says `created a task` (it named the ASSIGNEE and said `logged a task`, so an automation-raised task credited a person who had done nothing); a field edit is ONE `version` row rendering `<ActivityChanges>` — from → to, collapsed behind "+N more" — where it used to say `changed <field>` and drop both values | The server already sent `old_value`/`value` and the actor; the rail threw them away. New component: `frontend/src/tatva/ActivityChanges.vue` |
