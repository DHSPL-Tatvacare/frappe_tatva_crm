...
| `frontend/src/tatva/SmartViewEditor.vue` | NEW | Authoring drawer (create/edit/delete) — native `Dialog` 3-step flow reusing `Filter.vue` (fed `fields`) + `ColumnSettings.vue` (fed `fieldSource`); saves via `upsert_view`, deletes via `delete_view` behind the native confirm dialog |
| `frontend/src/tatva/smartViewPredicate.js` | NEW | Pure mapping between Filter.vue's emit dict and the composer predicate tree / ad-hoc filter array (no logic, no DOM) |
