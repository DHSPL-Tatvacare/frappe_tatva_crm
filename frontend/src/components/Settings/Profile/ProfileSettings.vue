<template>
        ...
        <div class="flex items-center justify-center gap-2">
          <div class="group relative !size-14">
            <Avatar
              ...
              :label="fullName"
            />
            <Tooltip :hoverDelay="0" placement="bottom" :text="profileTooltipText">
              <div
                class="z-1 absolute top-0 left-0 flex cursor-pointer items-center justify-center rounded-full !size-14"
            ...
            >
              <FeatherIcon name="x" class="size-3.5 cursor-pointer text-ink-gray-4" />
            </div>
          </div>
          <div class="flex flex-col gap-1">
            <div class="flex flex-col gap-1">
              <div v-if="!editName" class="flex items-center gap-1">
                ...
                <Button class="!px-1 !h-5" variant="ghost" @click="editFullName">
  ...
  <AvatarGallery
    v-model="showGallery"
    :current="user.doc?.user_image"
    @select="(url) => updateImage(url)"
  />
  <ChangePasswordModal
...
<script setup>
import AvatarGallery from '@/components/Settings/Profile/AvatarGallery.vue'
...
const profileTooltipText = computed(() => {
  ...
  return user.doc.user_image ? __('Change Photo') : __('Choose Photo')
})
